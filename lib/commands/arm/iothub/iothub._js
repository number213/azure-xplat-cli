...
exports.init = function (cli) {
  ...
  iotHub.command('create [resource-group] [name] [location] [sku-name] [units]')
    ...
    .option('-p --d2c-partitions [d2cPartitions]', $('The number of partitions for your device-to-cloud events. Range : F1 [2-2] Other SKUs [2-128]. Default : 2'))
    .option('-r --d2c-retention-time-in-days [d2cRetentionTimeInDays]', $('The number of days during which the device-to-cloud events will be maintained. Range : [1-7]. Default : 1.'))
    .option('-d --c2d-max-delivery-count [c2dMaxDeliveryCount]', $('The number of times the IoT Hub will attempt to deliver a cloud-to-device message. Range : [1-100]. Default : 10.'))
    .option('-c --c2d-ttl [c2dTtl]', $('The amount of time for which a cloud-to-device message is available for the device to consume before it is expired by IoT Hub, expressed in ISO8601 format. Range : [1 minute - 2 days]. Default : 1 hour (PT1H).'))
    .option('-f --feedback-lock-duration [feedbackLockDuration]', $('The lock duration for the feedback messages, expressed in ISO8601 format. Range : [5 seconds - 5 minutes]. Default : 1 minute (PT1M).'))
    .option('-F --feedback-ttl [feedbackTtl]', $('The amount of time a feedback message is available for a backend service to consume before it is expired by Iot Hub, expressed in ISO8601 format. Range : [1 minute - 2 days]. Default : 1 hour (PT1H).'))
    .option('-m --feedback-max-delivery-count [feedbackMaxDeliveryCount]', $('The number of times the IoT Hub will attempt to deliver a cloud-to-device feedback message to a backend service. Range : [1-100]. Default : 1.'))
  ...
  iotHubc2dproperties.command('set [resource-group] [name]')
    ...
    .option('-c --c2d-ttl [c2dTtl]', $('The amount of time for which a message is available for the device to consume before it is expired by IoT hub, between 1 and 48 hours'))
    ...
    .option('-F --feedback-ttl [feedbackTtl]', $('The amount of time a feedback message is available for the device to consume before it is expired by IoT hub, between 1 and 48 hours'))
