...
__.extend(ZoneFile.prototype, {
  ...
  checkCaaMultilineIssues: function(text) {
    var splitText = text.split('\n');
    splitText = splitText.map(function (line) { return line.replace('\r', ''); });

    var recRegex = /[^ ]+[ ]+[0-9]+[ ]+[\w]+[ ]+[A-Z]+.*/gim;
    var caaRegex = /[^ ]+[ ]+[0-9]+[ ]+[\w]+[ ]+CAA.*\(/gim;

    function checkRecord (lines) {
      if (!lines || !lines.length) {
        return;
      }
      if (lines[0].match(caaRegex) && lines.length > 1) {
        throw new Error(util.format($('Import file contains multiline record for CAA in:\n%s\nPlease change this to a single line and try again.'), lines.join('  \n')));
      }
    }
    ...
    for (i = 0; i < splitText.length; i++) {
      ...
      if (line.match(recRegex)) {
        checkRecord(recordLines);
        recordLines = [line];
      } else if (recordLines) {
  ...
  flatten: function (text) {
    this.checkCaaMultilineIssues(text);
    var bracketsPattern = /\([\s\S]*?\)\s+$/gim;
    var match = bracketsPattern.exec(text);
    while (match !== null) {
      match.replacement = match[0].replace(/\s+/gm, ' ').replace(/^\(/i, '').replace(/\)\s+$/i, '').trim();
  ...
  parseRR: function (rr, $origin, zoneName, $ttl, prevRrName) {
    ...
    while (rrTokens.length > 0) {
      ...
      if (validClasses.indexOf(token.toUpperCase()) !== -1 && (position <= 2)) {
        ...
        res.name = token;
      } else if (position === 1 && token === '0') {
        // Use default TTL
        res.ttl = constants.dnsZone.defTtl;
  ...
  convertToFQDN: function (value, $origin) {
    ...
    if (!$origin) {
      ...
      var fdqn = value + '.';
      if ($origin !== '.') {
        if (utils.stringEndsWith(value + '.', $origin, true)) {
          this.output.warn(util.format('DNS name "%s" looks like FQDN without terminating ".". Without termination it would be parsed as a relative name', value));
        }
        fdqn += $origin;
      }
      return fdqn;
  ...
  parseCAA: function (rr) {
    ...
    var warning = util.format('Record would be ignored. Record set: %s', rr.name);

    if (rr.data.length < 3) {
      self.output.warn(util.format('Failed to parse CAA record, it should contain all of the following: flags, tag, value. %s', warning));
      return;
    }

    var value = rr.data.slice(2).join(' ');

    if (value.indexOf(' ') !== -1 && !(value.startsWith('"') && value.endsWith('"'))) {
      self.output.warn(util.format('Failed to parse CAA value, values with spaces should be put in double-quotes. %s', warning));
      return;
    }
  ...
  generateRR: function (name, type, ttl, record) {
    ...
    switch (type) {
      ...
      case constants.dnsZone.CAA:
        var caaValue = record.value;
        // Escape double-quotes
        caaValue = caaValue.replace(/([\\]*)(")/g, function (match) {
          var count = (match.match(/\\/g) || []).length;
          if (count % 2 === 0) { match = match.replace('"', '\\"'); }
          return match;
        });
        // Enclose value in double-quotes if it contains spaces or escaped characters
        if (caaValue.indexOf(' ') !== -1 || caaValue.indexOf('\\') !== -1) {
          if (!caaValue.startsWith('"')) { caaValue = '"' + caaValue; }
          if (!caaValue.endsWith('"')) { caaValue = caaValue + '"'; }
        }
        rr += util.format('%s %s %s', record.flags, record.tag, caaValue);
        break;
